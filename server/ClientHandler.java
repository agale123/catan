		...
		while(true) {
			try {
				...
				if(o.getClass().equals(String.class)) {
					...
					if(ex.out[0] == BoardObject.type.ROAD) {
						if(_pool.getBoard().canBuyRoad(_index)) {
							_pool.broadcast(ex, this);
						}
					} else if(ex.out[0] == BoardObject.type.SETTLEMENT) {
						if(_pool.getBoard().canBuySettlement(_index)) {
							_pool.broadcast(ex, this);
						}
					} else if(ex.out[0] == BoardObject.type.CITY) {
						if(_pool.getBoard().canBuyCity(_index)) {
							_pool.broadcast(ex, this);
						}
					} else if(ex.out[0] == BoardObject.type.DEV) {
						if(_pool.getBoard().canBuyDev(_index)) {
							_pool.broadcase(ex, this);
						}
					} else {
						// TODO: Implement
					} 
